...
class App(ttk.Window):
    ...
    def create_widgets(self):
        self.main_title = ttk.Label(self, text="Send Mail From JSON", font=("Arial", 20))
        self.main_title.pack(pady=10)
        self.json_file = ttk.StringVar(value="Choose File")
        self.choose_file_button = ttk.Button(self, textvariable=self.json_file, command=self.choose_file, bootstyle="primary-outline")
        self.choose_file_button.pack(pady=10)
        self.send_button = ttk.Button(self, text="Send", command=self.send_mail, bootstyle="success-outline")
        self.send_button.pack(pady=10)
    
    def choose_file(self):
        self.json_file.set(fdg.askopenfilename(filetypes=[("JSON Files", "*.json")]))

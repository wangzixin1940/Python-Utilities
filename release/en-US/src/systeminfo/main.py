...
""")
    ...
    def match_choice_and_run(self, choice):
        ...
        match choice:
            ...
            case "7":
                print(
                    f"The name of the computer on the network: {
                        platform.node()}")
            case "8":
            ...
            case "0":
                print(
                    f"Linux system infomation: {
                        platform.freedesktop_os_release()}")
            case "a":
                print(
                    f"Python compile code and compile date: {
                        platform.python_build()}")
            case "b":
                print(f"Python compiler version: {platform.python_compiler()}")
            case "c":
                print(
                    f"Python implementer identity: {
                        platform.python_implementation()}")
            case "d":
                print(f"Python version: {platform.python_version()}")
            case "e":
                print(
                    f"Jython version of the interface: {
                        platform.java_ver()}")
            case "ab":
                print("""
